...
public class PropertyController {
	...
	@PostMapping
	public Page<PropertyResponse> listProperties(@RequestBody(required = false) @Valid PropertyRequest propertyRequest,
			@RequestParam(value = "page", defaultValue = "0") int page,
			@RequestParam(value = "size", defaultValue = "5") int size) {
		return propertyService.getProperties(page, size, propertyRequest);
	}
